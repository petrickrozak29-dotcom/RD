...
test('submission create -> approve -> visible', async ({ page, request }) => {
  ...
  const createRes = await request.post(`${apiBase}/api/submissions`, {
    data: {
  ...
  const login = await request.post(`${apiBase}/api/auth/login`, {
    data: { email: 'developermagelang45@gmail.com', password: 'potensimagelang45#' },
  });
  expect(login.ok()).toBeTruthy();
  const lb = await login.json();
  const token = lb.token;

  const patch = await request.patch(`${apiBase}/api/submissions/${created.id}/status`, {
  ...
  const foundInApi = records.some(
    (i: any) =>
      String(i.title) === String(created.title) ||
      String(i.id) === String(created.id) ||
      String(i.id) === `kuliner-${created.id}`
  );
  expect(foundInApi).toBeTruthy();

  // Then verify UI has rendered the item
  // Scope to the article card that contains the title to avoid ambiguous matches
  const locator = page.locator('article', { hasText: created.title }).first();
